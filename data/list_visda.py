import os
import random
p_path = os.path.join('E:\\Codes\\data\\visda-2017', 'train')
dir_list = os.listdir(p_path)
print(dir_list)

class_list = ["bicycle", "bus", "car", "motorcycle", "train", "truck", "unk"]
unk_list = ['aeroplane', 'horse', 'knife', 'person', 'plant', 'skateboard']
    ...
    if not '.txt' in direc:
        ...
        for i, file in enumerate(files):
            if direc in class_list:
                ...
                continue
p_path = os.path.join('E:\\Codes\\data\\visda-2017', 'validation')
dir_list = os.listdir(p_path)
print(dir_list)
for k, direc in enumerate(dir_list):
    if not '.txt' in direc:
